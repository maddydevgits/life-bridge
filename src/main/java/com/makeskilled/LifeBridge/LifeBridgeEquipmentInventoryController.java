...
public class LifeBridgeEquipmentInventoryController {

    @Autowired
    private LifeBridgeEquipmentInventoryRepository equipmentRepo;

    // List all equipment inventory
    @GetMapping("/list")
    public String listEquipment(HttpSession session,Model model) {
        String username = (String) session.getAttribute("username");
        List<LifeBridgeEquipmentInventoryModel> equipmentList = equipmentRepo.findAll();
    ...
    public String addEquipmentForm(HttpSession session,Model model) {
        String username = (String) session.getAttribute("username");
        model.addAttribute("username", username);
        model.addAttribute("equipmentInventory", new LifeBridgeEquipmentInventoryModel());
        return "add_equipment";  // Corresponding Thymeleaf template for the form
    }

    // Save new equipment
    @PostMapping("/save")
    public String saveEquipment(@ModelAttribute("equipmentInventory") LifeBridgeEquipmentInventoryModel equipmentInventory) {
    ...
    public String updateEquipmentForm(HttpSession session,@PathVariable("id") Long id, Model model) {
        String username = (String) session.getAttribute("username");
        model.addAttribute("username", username);
        LifeBridgeEquipmentInventoryModel equipment = equipmentRepo.findById(id).orElse(null);
    ...
    @PostMapping("/update")
    public String updateEquipment(@ModelAttribute("equipmentInventory") LifeBridgeEquipmentInventoryModel equipmentInventory) {
        equipmentRepo.save(equipmentInventory);
        return "redirect:/equipment/list";
    }
}
